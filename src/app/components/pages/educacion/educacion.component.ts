...
export class EducacionComponent implements OnInit {
  ...
  constructor(public router: Router, private eduSvc: Service, private snackBar: MatSnackBar) {
  }
  ...
  ngOnInit() {
    ...
    this.isActive = false;
    this.collapsedE = false;
    this.pushRightClass = 'push-right';
    this.eduSvc
      .getDataxFiltro('video', (ref) =>
        ref
          .where('estado', '==', true)
          .where('codigoSeccion', '==', 2)
      ...
      .subscribe((responseC) => {
        if (responseC.length > 0 && !this.controlUV) {
          ...
          if (videoA.length > 0) {
            ...
            this.showMenuE = videoA[0].idEtapa;
          } else {
            ...
            this.showMenuE = this.videos[0].idEtapa;
          }
          this.controlUV.fechaUltimaVisita = new Date();
          this.controlUV.idVideoActual = this.filterVideo;
          this.eduSvc.preAddUpdateControlUVById(this.controlUV);
          ...
          if (responseC.length === 0 && !this.controlUV) {
            ...
            this.controlUV = this.eduSvc.AddControlUV(this.controlUV);
            if (this.videos[1]) {
              this.nombreS = `${this.videos[0].posicionVPantalla + 1}. ${this.videos[this.videos[0].posicionVPantalla].nombre}`;
            }
            this.filterVideo = this.videos[0].id;
  ...
  addExpandClass(element: any) {
    if (element === this.showMenuE) {
      this.showMenuE = 0;
    } else {
      this.showMenuE = element;
    }
  }
  eventCalled() {
  ...
  toggleCollapsed() {
    this.collapsedE = !this.collapsedE;
  }
  getVideos(idEtapa) {
  ...
  getPrev(video: any) {
    ...
    this.showMenuE = this.videos[video.posicionVPantalla - 2].idEtapa;
    this.controlUV.segundo = 0;
    this.eduSvc.preAddUpdateControlUVById(this.controlUV);
    this.segundoA = 0;
  }
  ...
  moveNext(video: any) {
    ...
    this.showMenuE = this.videos[video.posicionVPantalla].idEtapa;
    this.controlUV.segundo = 0;
    this.eduSvc.preAddUpdateControlUVById(this.controlUV);
    this.segundoA = 0;
  }
  videoTerminado(video: any) {
    ...
    if (video.posicionVPantalla <= this.videosL - 1) {
      ...
      if (this.videosL === vistosE) {
        this.snackBar.open(
          'Felicidades has completado todo el módulo de educación!',
          'Cerrar',
          {
            duration: 5000,
            horizontalPosition: 'center',
