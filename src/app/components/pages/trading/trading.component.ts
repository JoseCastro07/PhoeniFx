...
export class TradingComponent implements OnInit {
  ...
  constructor(public router: Router, private tradSvc: Service, private snackBar: MatSnackBar) {
  }
      ...
      .subscribe((responseC) => {
        if (responseC.length > 0 && !this.controlUV) {
          ...
          if (videoA.length > 0) {
            ...
            this.showMenuT = videoA[0].idEtapa;
          } else {
            ...
            this.showMenuT = this.videos[0].idEtapa;
          }
          this.controlUV.fechaUltimaVisita = new Date();
          this.controlUV.idVideoActual = this.filterVideo;
          this.tradSvc.preAddUpdateControlUVById(this.controlUV);
          ...
          if (responseC.length === 0 && !this.controlUV) {
            ...
            this.controlUV = this.tradSvc.AddControlUV(this.controlUV);
            if (this.videos[1]) {
              this.nombreS = `${this.videos[0].posicionVPantalla + 1}. ${this.videos[this.videos[0].posicionVPantalla].nombre}`;
            }
            this.filterVideo = this.videos[0].id;
  ...
  addExpandClass(element: any) {
    if (element === this.showMenuT) {
      this.showMenuT = 0;
    } else {
      this.showMenuT = element;
    }
  }
  eventCalled() {
  ...
  toggleCollapsed() {
    this.collapsedT = !this.collapsedT;
  }
  getVideos(idEtapa) {
  ...
  getPrev(video: any) {
    ...
    this.showMenuT = this.videos[video.posicionVPantalla - 2].idEtapa;
    this.controlUV.segundo = 0;
    this.tradSvc.preAddUpdateControlUVById(this.controlUV);
    this.segundoA = 0;
  }
  ...
  moveNext(video: any) {
    ...
    this.showMenuT = this.videos[video.posicionVPantalla].idEtapa;
    this.controlUV.segundo = 0;
    this.tradSvc.preAddUpdateControlUVById(this.controlUV);
    this.segundoA = 0;
  }
  videoTerminado(video: any) {
    ...
    if (video.posicionVPantalla <= this.videosL - 1) {
      ...
      let vistosT = 0;
      this.etapas.map(m => {
        vistosT += m.totalVistosE;
      });
      if (this.videosL === vistosT) {
        this.snackBar.open(
          'Felicidades has completado todo el módulo de trading!',
          'Cerrar',
          {
            duration: 5000,
            horizontalPosition: 'center',
  ...
  porcentajeResultado() {
    ...
    this.porcentajeVideosV = Math.round(vistosT * 100 / this.videosL);
    ...
    this.resultado = `${vistosT} ${aux1} de ${this.videosL} ${aux2}.`;
  }
  guardarS(videoP) {
