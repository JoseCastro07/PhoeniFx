...
export class WebinarComponent implements OnInit {
  ...
  constructor(public router: Router, private webSvc: Service, private snackBar: MatSnackBar) {
  }
  ...
  ngOnInit() {
    ...
    this.isActive = false;
    this.collapsedW = false;
    this.pushRightClass = 'push-right';
    this.webSvc
      .getDataxFiltro('video', (ref) =>
        ref
          .where('estado', '==', true)
          .where('codigoSeccion', '==', 4)
      ...
      .subscribe((responseC) => {
        if (responseC.length > 0 && !this.controlUV) {
          ...
          if (videoA.length > 0) {
            ...
            this.showMenuW = videoA[0].idEtapa;
          } else {
            ...
            this.showMenuW = this.videos[0].idEtapa;
          }
          this.controlUV.fechaUltimaVisita = new Date();
          this.controlUV.idVideoActual = this.filterVideo;
          this.webSvc.preAddUpdateControlUVById(this.controlUV);
          ...
          if (responseC.length === 0 && !this.controlUV) {
            ...
            this.controlUV = this.webSvc.AddControlUV(this.controlUV);
            if (this.videos[1]) {
              //this.nombreS = `${this.videos[0].posicionVPantalla + 1}. ${this.videos[this.videos[0].posicionVPantalla].nombre}`;
              this.nombreS = `${this.videos[this.videos[0].posicionVPantalla].nombre}`;
            }
  ...
  addExpandClass(element: any) {
    if (element === this.showMenuW) {
      this.showMenuW = 0;
    } else {
      this.showMenuW = element;
    }
  }
  eventCalled() {
  ...
  toggleCollapsed() {
    this.collapsedW = !this.collapsedW;
  }
  getVideos(idEtapa) {
  ...
  getPrev(video: any) {
    ...
    this.showMenuW = this.videos[video.posicionVPantalla - 2].idEtapa;
    this.controlUV.segundo = 0;
    this.webSvc.preAddUpdateControlUVById(this.controlUV);
    this.segundoA = 0;
  }
  ...
  moveNext(video: any) {
    ...
    this.showMenuW = this.videos[video.posicionVPantalla].idEtapa;
    this.controlUV.segundo = 0;
    this.webSvc.preAddUpdateControlUVById(this.controlUV);
    this.segundoA = 0;
  }
  videoTerminado(video: any) {
    ...
    if (video.posicionVPantalla <= this.videosL - 1) {
      ...
      let vistosW = 0;
      this.etapas.map(m => {
        vistosW += m.totalVistosE;
      });
      if (this.videosL === vistosW) {
        this.snackBar.open(
          'Felicidades has completado todo el módulo de webinar!',
          'Cerrar',
          {
            duration: 5000,
            horizontalPosition: 'center',
  ...
  porcentajeResultado() {
    ...
    this.porcentajeVideosV = Math.round(vistosW * 100 / this.videosL);
    ...
    this.resultado = `${vistosW} ${aux1} de ${this.videosL} ${aux2}.`;
  }
  guardarS(videoP) {
